...
def showCube(cube):
    faceValue = 198
    posicoes = [[1*faceValue + 10, 0*faceValue + 5], [0*faceValue + 5, 1*faceValue + 10], [1*faceValue + 10, 1*faceValue + 10],
                [2*faceValue + 15, 1*faceValue + 10], [3*faceValue + 20, 1*faceValue + 10], [1*faceValue + 10, 2*faceValue + 15]]
    for face in range(6):
        for coluna in range(3):
            for linha in range(3):
                color = letterColor(cube[face][coluna][linha])
                pygame.draw.rect(
                    screen, color, (posicoes[face][0], posicoes[face][1], faceValue/3, faceValue/3))
...
def embaralhar():
    ...
    for x in range(19):
        ...
        tempMove = movementValues[randomMovimentos] + \
            valoresSecundarios[randomSecundarios]
...
fontesys = pygame.font.SysFont(font, 20)
txt_tela = fontesys.render('New Scramble', 0.5, (75, 139, 59))
quad_rect = pygame.Rect(600, 90, 100, 20)
txt_rect = txt_tela.get_rect(center = quad_rect.center)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if quad_rect.collidepoint(mouse_pos):
                resetCube()
                embaralhar()
                pygame.draw.rect(screen, (46, 46, 46), (0, 610, 850, 50))
    ...
    pygame.draw.rect(screen, 'yellow', quad_rect)
    screen.blit(txt_tela, txt_rect)
